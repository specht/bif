...
async function loadSha256() {
  return import(pathToFileURL(path.join(process.cwd(), 'dev/browser-sha256.js')));
}
...
test('analysis manifest verification works without global Web Crypto', async () => {
  ...
  try {
    const { createBrowserAnalysisClient } = await import(pathToFileURL(path.join(process.cwd(), 'dev/browser-analysis-client.js')));
