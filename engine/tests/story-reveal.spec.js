...
async function openChoiceFixture(page, mode = 'game') {
  await page.route(/\/config\.js(?:\?.*)?$/, route => route.fulfill({
    contentType: 'text/javascript', body: "export const path = 'engine/test-fixtures/choice-results/pages';",
  }));
...
test('semantic planner keeps structural blocks whole and splits only plain long prose', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const reveal = await import('/engine/runtime/modules/browser-story-reveal.js');
...
test('new page commits before reveal, preserves prior transcript, and restores immediately', async ({ page }) => {
  await page.route(/\/config\.js(?:\?.*)?$/, route => route.fulfill({
    contentType: 'text/javascript', body: "export const path = 'engine/test-fixtures/progressive-page';",
  }));
  await page.route(/\/engine/test-fixtures\/progressive-page\/1\.md\?.*/, route => route.fulfill({
    contentType: 'text/markdown', body: '# Before\n\nPrevious transcript text.\n\n- [Open the new page.](2)',
  }));
  await page.route(/\/engine/test-fixtures\/progressive-page\/2\.md\?.*/, route => route.fulfill({
  ...
  await page.route(/\/engine/test-fixtures\/progressive-page\/3\.md\?.*/, route => route.fulfill({
