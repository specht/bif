...
test('current analysis renders the authoritative graph', async ({ page }) => {
  await configureFixture(page, 'engine/test-fixtures/authoring-graph/complete-project');
...
test('missing analysis is actionable and retry recovers', async ({ page }) => {
  const { publication } = await configureFixture(page, 'engine/test-fixtures/authoring-graph/complete-project');
  let missing = true;
...
test('strict schema rejects unsupported and malformed publications safely', async ({ page }) => {
  ...
  const results = await page.evaluate(async () => {
    const { validateBrowserAnalysis } = await import('/engine/dev/browser-analysis-client.js');
...
test('browser schema expectation equals the authoritative publisher constant and accepts both sources', async ({ page }) => {
  ...
  const result = await page.evaluate(async publications => {
    const client = await import('/engine/dev/browser-analysis-client.js');
    return {
      expected: client.SUPPORTED_SCHEMA_VERSION,
      valid: publications.map(value => client.validateBrowserAnalysis(value).valid),
    };
  }, [
    buildBrowserAnalysisPublication(await analyzeStory(path.join(root, 'engine/test-fixtures/authoring-graph/complete-project')), {
...
test('local mode preference is path-scoped and production ignores dev override', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const mode = await import('/engine/runtime/modules/browser-mode.js');
