...
async function openFixture(page, mode = 'game') {
  const analysis = buildBrowserAnalysisPublication(await analyzeStory(path.join(process.cwd(), 'engine/test-fixtures/choice-results')));
  await page.route(/\/config\.js(?:\?.*)?$/, route => route.fulfill({
    contentType: 'text/javascript',
    body: "export const path = 'engine/test-fixtures/choice-results/pages';",
  }));
...
test('ordinary loose Markdown list paragraphs remain intact', async ({ page }) => {
  await page.route(/\/config\.js(?:\?.*)?$/, route => route.fulfill({
    contentType: 'text/javascript', body: "export const path = 'engine/test-fixtures/loose-list-choice';",
  }));
  await page.route(/\/engine/test-fixtures\/loose-list-choice\/1\.md\?.*/, route => route.fulfill({
...
test('local scroll target is finite, clamped, and distinguishes short and tall turns', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const { localTurnScrollTarget } = await import('/engine/runtime/modules/browser-story-scroll.js');
    return {
...
test('a tall local answer anchors its committed turn instead of skipping to the end', async ({ page }) => {
  await page.setViewportSize({ width: 900, height: 360 });
  await page.route(/\/config\.js(?:\?.*)?$/, route => route.fulfill({
    contentType: 'text/javascript', body: "export const path = 'engine/test-fixtures/local-scroll';",
  }));
  const longAnswer = Array.from({ length: 18 }, (_, index) => `Answer paragraph ${index + 1}.`).join('\n\n');
  await page.route(/\/engine/test-fixtures\/local-scroll\/1\.md\?.*/, route => route.fulfill({
    contentType: 'text/markdown',
