...
async function useStory(page, storyPath = 'engine/test-fixtures/player-basic/pages') {
...
test('local choices and context are cleared by the shared restart action', async ({ page }) => {
  await useStory(page, 'engine/test-fixtures/choice-results/pages');
  await page.goto('/?mode=dev');
...
test('shared restart reset contains no browser-native confirmation', async () => {
  const source = await readFile(path.join(root, 'engine/runtime/script.js'), 'utf8');
