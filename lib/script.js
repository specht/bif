...
function updateRestartControl() {
    ...
    restartLabel.textContent = 'Spiel neu starten';
...
export async function init() {
    ...
    if (devMode) {
        ...
        createGraphToolbar();
        document.querySelector('.development-toolbar-actions').append(document.querySelector('#bu_reset_game'));
        updateRestartControl();
        graphProblems = createProblemsView({
            graphContainer: el.graphContainer,
            stateContainer: el.stateContainer,
            uiState: developmentUiState,
            onSelect(structuralId) {
                const element = unifiedGraphView?.select(structuralId);
                if (element) focusOnElement(element);
            },
        });
...
function createGraphToolbar() {
    ...
    toolbar.append(fit, autoFollowButton);
    el.graphContainer.before(toolbar);
}
