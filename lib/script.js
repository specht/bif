...
async function navigateFromGraphEdge(edgeId) {
    const control = [...el.content.querySelectorAll('.story-choice')].find(item => item.dataset.graphEdgeId === edgeId);
    if (control && !control.classList.contains('chosen') && !control.classList.contains('dismissed')) control.click();
}

async function rewindFromGraph(page) {
