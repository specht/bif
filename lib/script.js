...
import { followViewBoxTarget, panViewBox, pinchViewBox, zoomViewBoxAt, interpolateViewBox } from '/lib/browser-graph-viewport.js';
...
async function processDOM(inputRoot, outputRoot, pageMetadata, generation = sessionGeneration) {
    let scriptIndex = 0;
    ...
    function sourceMetadata(source) {
        ...
        return {
            ...
            line: Number.isInteger(sourceStartLine) ? sourceStartLine + leadingLineBreaks : sourceStartLine,
            source,
        };
    }
...
function decodeSession(hash = window.location.hash) {
    ...
    if (!Number.isFinite(seed) || (session.length > 1 && session[1] !== '1')) return null;
    return session;
}
        ...
        .then(async data => {
            ...
            for (let link of appendedPassage.querySelectorAll('a')) {
                ...
                if (href.indexOf('/') < 0) {
                    ...
                    if (page) {
                        const control = link;
                        control.dataset.pageTarget = `${page}`;
            ...
            mapPassageChoices(appendedPassage, `${page}`);
            updateRestartControl();
...
async function rewindFromGraph(page, origin = 'pointer') {
    ...
    try {
        await restoreSession(history.slice(0, index + 1));
        requestSessionWrite('push');
    } finally {
        el.body.classList.remove('skip-animations');
    }
...
let followRequestFrame = null;
const FOLLOW_VIEWPORT_MARGIN = 0.15;
...
function cancelFollowRequest() {
    if (followRequestFrame !== null) cancelAnimationFrame(followRequestFrame);
    followRequestFrame = null;
}
...
function setAutoFollow(value, { persist = true } = {}) {
    autoFollow = Boolean(value);
    autoFollowButton?.setAttribute('aria-pressed', String(autoFollow));
    autoFollowButton?.classList.toggle('active', autoFollow);
    if (persist) developmentUiState?.set('graphAutoFollow', autoFollow, { immediate: true });
}
...
function followCurrentGraphNode() {
    if (!autoFollow) return;
    cancelFollowRequest();
    followRequestFrame = requestAnimationFrame(() => {
        followRequestFrame = null;
        if (!autoFollow || !window.svg) return;
        const pageId = history.at(-1);
        const element = pageId ? el.graphContainer.querySelector(`#node_${CSS.escape(String(pageId))}`) : null;
        if (!element) return;
        const bounds = getBBoxInSVGCoords(element, window.svg);
        const target = followViewBoxTarget(viewBox, bounds, {
            margin: FOLLOW_VIEWPORT_MARGIN,
            epsilon: FOLLOW_EPSILON,
            minWidth: viewportLimits.minWidth,
            maxWidth: Math.min(viewportLimits.maxWidth, viewBox.width * 2),
        });
        if (target) animateViewBox(target, 260);
    });
}
...
function createGraphToolbar() {
    ...
    autoFollowButton = document.createElement('button');
    autoFollowButton.type = 'button';
    autoFollowButton.className = 'icon-text';
    autoFollowButton.title = 'Auto-follow story progress';
    autoFollowButton.append(createIcon('target-arrow'), Object.assign(document.createElement('span'), { className: 'icon-label', textContent: 'Auto-follow' }));
    setAutoFollow(Boolean(developmentUiState?.get('graphAutoFollow', false)), { persist: false });
    ...
    autoFollowButton.addEventListener('click', () => {
        ...
        if (autoFollow) followCurrentGraphNode();
    });
    document.querySelector('.development-toolbar-actions').append(fit, autoFollowButton);
}
