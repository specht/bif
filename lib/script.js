...
async function processStaticChoice(control, generation = sessionGeneration, { record = true } = {}) {
    ...
    try {
        ...
        if (choice.local) {
            ...
            const selected = passage.querySelector(`[data-choice-id="${CSS.escape(choice.identity)}"]`);
            const liveSet = selected?.closest('.live-choice-set');
            const followsLocalTurns = [...runtime.committed.keys()].some(choiceId =>
                choiceId !== choice.identity && runtime.choiceById.get(choiceId)?.local);
            if (selected && liveSet && followsLocalTurns) {
                const item = selected.closest('li');
                const turns = passage.querySelector(':scope > .committed-choice-turns') || document.createElement('div');
                if (!turns.isConnected) {
                    turns.className = 'committed-choice-turns';
                    liveSet.before(turns);
                }
                const turn = document.createElement('div');
                turn.className = 'committed-choice-turn';
                turn.appendChild(item);
                turns.appendChild(turn);
                installCommittedResult(selected, runtime.committed.get(choice.identity));
                liveSet.remove();
            } else if (selected && liveSet) {
                installCommittedResult(selected, runtime.committed.get(choice.identity));
                for (const candidate of liveSet.querySelectorAll('.story-choice')) {
                    deactivateChoice(candidate, candidate === selected);
                }
            }
