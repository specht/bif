...
function presentChoice(choices, generation = sessionGeneration) {
...
function createContextProxy(generation = sessionGeneration) {
    ...
    get(target, key) {
        if (key in target) {
            ...
            return choices => presentChoice(choices, generation);
...
function runInContext(code, generation = sessionGeneration) {
    let result = AsyncFunction('ctx', `with (ctx) { ${code} }`)(createContextProxy(generation));
    updateStateDisplay();
    return result;
}

function trackStoryTask(promise, sourceInfo, generation = sessionGeneration) {
    const task = Promise.resolve(promise);
    activeStoryTasks.add(task);
    task.catch(error => {
        if (error?.name === 'AbortError' || sessionWasAbandoned(generation)) return;
        console.error(`Story script failed (${sourceInfo}):`, error);
        appendSection(`Fehler im Seitenskript ${sourceInfo}: ${error.message}`);
    }).finally(() => {
...
function processDOM(inputRoot, generation = sessionGeneration) {
    ...
    function processNode(node, outputParent) {
        ...
        if (node.tagName.toLowerCase() === 'script') {
            if (checkCondition(node)) {
                trackStoryTask(runInContext(node.textContent, generation), 'in der aktuellen Seite', generation);
            }
            return;
        }
...
async function appendPage(page, generation = sessionGeneration) {
    await fetch(`/${path}/${page}.md?${cache_buster}`)
        ...
        .then(data => {
            ...
            processDOM(doc.body, generation);
...
function navigateToPage(page, generation = sessionGeneration) {
    ...
    const navigation = navigationQueue.then(async () => {
        ...
        try {
            ...
            throw new Error(`Could not navigate to page ${pageId}: ${error.message}`, { cause: error });
        }
    });
...
async function turnToPage(page, generation = sessionGeneration) {
    ...
    if (deferred) {
        ...
        for (let option of choiceDiv.querySelectorAll('.pagelink')) {
            if (option !== selectedChoice) option.classList.add('dismissed');
        }
        choiceDiv = null;
