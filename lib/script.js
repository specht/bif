...
function runInContext(code) {
    let result = AsyncFunction('ctx', `with (ctx) { ${code} }`)(contextProxy);
    updateStateDisplay();
    return result;
}

function trackStoryTask(promise, sourceInfo) {
    ...
    function processNode(node, outputParent) {
        ...
        if (node.tagName.toLowerCase() === 'script') {
            if (checkCondition(node)) {
                trackStoryTask(runInContext(node.textContent), 'in der aktuellen Seite');
            }
            return;
        }
...
export async function init() {
    ...
    if (history.length < 3) {
        ...
        for (const [pageId, request] of replayNavigationRequests) {
            replayNavigationRequests.delete(pageId);
            navigateToPage(pageId).then(request.resolve, request.reject);
        }
    }
