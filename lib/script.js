...
function runInContext(code, metadata, generation = sessionGeneration) {
    ...
    let result = executable(createContextProxy(generation));
    updateStateDisplay();
    return result;
}
...
function processDOM(inputRoot, pageMetadata, generation = sessionGeneration) {
    let scriptIndex = 0;
    ...
    function processNode(node, outputParent) {
        ...
        if (node.tagName.toLowerCase() === 'script') {
            ...
            try {
                trackStoryTask(runInContext(code, metadata, generation), metadata, generation);
            } catch (error) {
...
async function appendPage(page, generation = sessionGeneration) {
    ...
    await fetch(`/${pagePath}?${cache_buster}`)
        ...
        .then(data => {
            ...
            processDOM(doc.body, { pageId: `${page}`, pagePath, markdown: data }, generation);
