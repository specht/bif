...
function reportStoryError(error, outputParent = el.content) {
    ...
    if (!devMode) {
        ...
        if (error.kind === 'page-load') {
            block.append('This passage could not be completed.', document.createElement('br'), `Missing page: ${error.pagePath}.`);
            outputParent.appendChild(block);
            return;
        }
        const location = [error.pagePath, error.line ? `on line ${error.line}` : null].filter(Boolean).join(' ');
        const kind = error.kind === 'condition' ? 'condition' : error.kind === 'expression' ? 'expression' : 'script';
        block.append('This passage could not be completed.');
        if (location) block.append(
            document.createElement('br'), `There is a ${kind} error in ${location}.`,
            document.createElement('br'), 'See Problems below for the highlighted source.',
        );
    }
    outputParent.appendChild(block);
}
...
function installPanAndZoomHandler(svg) {
    ...
    window.svg.addEventListener('mousedown', mouseDown);
    ...
    removePanAndZoomHandlers = () => {
        installedSvg.removeEventListener('mousedown', mouseDown);
        document.removeEventListener('mousemove', handlePan);
        document.removeEventListener('mouseup', mouseUp);
        installedSvg.removeEventListener('wheel', zoom);
        installedSvg.removeEventListener('touchstart', handleTouchStart);
        installedSvg.removeEventListener('touchmove', handleTouchMove);
        installedSvg.removeEventListener('touchend', handleTouchEnd);
    };
}
