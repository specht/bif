...
function nodeStatement(node) {
    const marker = node.errorCount ? ' ✕' : node.warningCount ? ' ⚠' : '';
...
export function buildPlayerGraphDot(structure) {
    ...
    for (const edge of structure.edges) {
        ...
        const marker = edge.broken || edge.diagnostics.some(item => item.severity === 'error') ? ', xlabel="✕"' : edge.diagnostics.some(item => item.severity === 'warning') ? ', xlabel="⚠"' : '';
