        ...
        for (const [file, fileProblems] of groups) {
            ...
            for (const problem of fileProblems) {
                ...
                if (sourceUri) {
                    ...
                    open.className = 'problem-open-source icon-text';
                    open.href = sourceUri;
                    const openLabel = document.createElement('span');
                    openLabel.className = 'icon-label';
                    openLabel.textContent = 'VS Code';
                    open.append(createIcon('brand-vscode'), openLabel);
                    open.title = 'Open in VS Code';
                    open.setAttribute('aria-label', `Open ${problem.diagnostic.file} at line ${problem.diagnostic.line} in VS Code`);
                    open.addEventListener('click', event => event.stopPropagation());
                    rowHeading.append(control, open);
                } else {
