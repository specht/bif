...
function publicMessage(diagnostic) {
    let message = String(diagnostic.message || '');
    if (Number.isInteger(diagnostic.scriptIndex)) message = message.replace(new RegExp(`^Script ${diagnostic.scriptIndex}:\\s*`), '');
    if (Number.isInteger(diagnostic.scriptLine) && Number.isInteger(diagnostic.scriptColumn)) {
        message = message.replace(new RegExp(`\\s*\\(${diagnostic.scriptLine}:${diagnostic.scriptColumn}\\)$`), '');
    }
    return message;
}

function displayMessage(diagnostic) {
    const message = publicMessage(diagnostic);
