...
function compactLocationText(diagnostic) {
    if (!Number.isInteger(diagnostic.line)) return '';
    return Number.isInteger(diagnostic.column) ? `${diagnostic.line}:${diagnostic.column}` : String(diagnostic.line);
}
    ...
    function render(structure) {
        ...
        for (const [file, fileProblems] of groups) {
            ...
            for (const problem of fileProblems) {
                ...
                severity.append(createIcon(error ? 'x' : 'alert-triangle'), document.createTextNode(error ? 'Error' : 'Warning'));
                ...
                location.textContent = compactLocationText(problem.diagnostic);
                location.title = locationText(problem.diagnostic);
                ...
                if (sourceUri) {
                    ...
                    open.href = sourceUri;
                    open.replaceChildren(createIcon('brand-visual-studio'));
